...
typedef struct {
    double *pos_cmd;           /* RPI: commanded position */
    double *teleop_vel_cmd;    /* RPI: commanded velocity */
    double *teleop_pos_cmd;    /* RPI: teleop traj planner pos cmd */
    double *teleop_vel_lim;    /* RPI: teleop traj planner vel limit */
    int   *teleop_tp_enable;  /* RPI: teleop traj planner is running */

    int   *ajog_counts;       /* WPI: jogwheel position input */
    int   *ajog_enable;       /* RPI: enable jogwheel */
    double *ajog_scale;        /* RPI: distance to jog on each count */
    double *ajog_accel_fraction;  /* RPI: to limit wheel jog accel */
    int   *ajog_vel_mode;     /* RPI: true for "velocity mode" jogwheel */
    int   *kb_ajog_active;    /* RPI: executing keyboard jog */
    int   *wheel_ajog_active; /* RPI: executing handwheel jog */

    int   *eoffset_enable;
    int   *eoffset_clear;
    int   *eoffset_counts;
    double *eoffset_scale;
    double *external_offset;
    double *external_offset_requested;
} axis_hal_t;
